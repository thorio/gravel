...
fn get_exit(config: &Config) -> Box<dyn Hit> {
    ...
    Box::new(SimpleHit::new(data, |_hit, sender| {
        sender.send(FrontendMessage::Exit).unwrap();
    }))
}

fn get_lock(config: &Config) -> Box<dyn Hit> {
    let data = HitData::new(&config.lock.title, &config.lock.subtitle);
    Box::new(SimpleHit::new_extra(data, config.lock.clone(), |_hit, sender| {
        implementation::lock(_hit.get_extra_data());
...
fn get_logout(config: &Config) -> Box<dyn Hit> {
    ...
    Box::new(SimpleHit::new_extra(data, config.logout.clone(), |_hit, sender| {
        implementation::logout(_hit.get_extra_data());
        sender.send(FrontendMessage::Hide).unwrap();
    }))
}
...
fn get_restart(config: &Config) -> Box<dyn Hit> {
    ...
    Box::new(SimpleHit::new_extra(data, config.restart.clone(), |_hit, sender| {
        implementation::restart(_hit.get_extra_data());
        sender.send(FrontendMessage::Hide).unwrap();
    }))
}
...
fn get_shutdown(config: &Config) -> Box<dyn Hit> {
    ...
    Box::new(SimpleHit::new_extra(data, config.shutdown.clone(), |_hit, sender| {
        implementation::shutdown(_hit.get_extra_data());
        sender.send(FrontendMessage::Hide).unwrap();
    }))
}
...
fn get_sleep(config: &Config) -> Box<dyn Hit> {
    ...
    Box::new(SimpleHit::new_extra(data, config.sleep.clone(), |_hit, sender| {
        implementation::sleep(_hit.get_extra_data());
        sender.send(FrontendMessage::Hide).unwrap();
    }))
}
